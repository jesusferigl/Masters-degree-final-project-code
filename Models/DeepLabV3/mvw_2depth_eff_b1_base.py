...
experiment.set_name('MVW-2Depth-Eff-b1-Base')
...
class MultiviewNet(torch.nn.Module):
    def __init__(self, name_second_network):
        ...
        self.modelo = smp.DeepLabV3Plus(encoder_name = 'timm-efficientnet-b1', encoder_weights = 'noisy-student', in_channels=9, classes=2)
    def forward(self, x1):
...
def entrena(red):
    ...
    torch.save(mejor_modelo, 'mejor_modelo_MVW-2Depth-Eff-b1-Base')
    torch.save(red, 'ultimo_modelo_MVW-2Depth-Eff-b1-Base')
