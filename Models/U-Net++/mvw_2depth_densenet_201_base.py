...
experiment.set_name('MVW-2Depth-DenseNet-201-Base')
...
class MultiviewNet(torch.nn.Module):
    def __init__(self, name_second_network):
        ...
        self.modelo = smp.UnetPlusPlus(encoder_name='densenet201', encoder_weights='imagenet', in_channels=9, classes=2).to('cuda')
    def forward(self, x1):
...
def entrena(red):
    ...
    torch.save(mejor_modelo, 'mejor_modelo_MVW-2Depth-DenseNet-201-Base')
    torch.save(red, 'ultimo_modelo_MVW-2Depth-DenseNet-201-Base')
...
#experiment.log_notebook('MVW-2Depth-DenseNet-201-Base.py')
experiment.end()
